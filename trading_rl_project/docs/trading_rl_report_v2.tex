...
El mundo de las \textbf{criptomonedas}\footnote{Activos digitales descentralizados cuya volatilidad genera riesgos y oportunidades de inversión.} es conocido por ser como una montaña rusa. Muchas veces, estos cambios no ocurren solo por números, sino por el factor humano. Tradicionalmente, los programas de \textbf{trading}\footnote{Negociación estratégica de activos financieros para obtener rentabilidad.} solo miran estadísticas, pero olvidan el sentimiento que mueve el mercado.

Este proyecto propone crear una Inteligencia Artificial (IA) que integre la psicología del mercado mediante el \textbf{Aprendizaje por Refuerzo}\footnote{Rama de la IA donde un agente aprende a tomar decisiones mediante un sistema de premios y castigos.}, entrenando al agente para reaccionar ante el pánico o la euforia colectiva.

Nuestra IA operará con la moneda de \textbf{Binance}\footnote{Ecosistema global que actúa como el entorno de operación para nuestro agente.} (\textbf{BNB}\footnote{Activo nativo de Binance utilizado como instrumento principal de inversión.}) y un índice de Miedo y Codicia. El objetivo es que la IA aprenda a ser precavida cuando hay pánico y estratégica cuando hay euforia, buscando un crecimiento seguro del capital.
...
\begin{enumerate}
    \item \textbf{El Agente}\footnote{Es el componente de Inteligencia Artificial que actúa como el ``sujeto'' del aprendizaje. Es el cerebro que observa el mercado, evalúa el riesgo y toma decisiones basándose en una estrategia que perfecciona a través de la experiencia.}\textbf{:} Es el ``cerebro'' de la IA, el que toma las decisiones.
    ...
    \item \textbf{La Acción}\footnote{Se refiere a cada una de las decisiones que el agente puede ejecutar en un momento determinado. En nuestro caso, el agente elige cada hora entre Comprar, Vender o Esperar (no hacer nada), buscando siempre la opción que maximice su beneficio a largo plazo.}\textbf{:} Es la decisión que toma el agente (Comprar, Vender o Esperar).
    \item \textbf{La Recompensa ($R_t$):} Es el premio o castigo que recibe el agente por su acción.
\end{enumerate}

A partir de ahora, exploraremos una a una estas piezas para entender cómo nuestra IA se convirtió en una experta del mercado bajo el marco de un \textbf{Proceso de Decisión de Markov}\footnote{Es el marco conceptual que define cómo un agente interactúa con su entorno basándose en la idea de que el futuro solo depende del estado actual.} \textbf{(MDP)}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{¿Qué es?} Técnicamente se llama \textbf{factor de descuento}\footnote{Es un parámetro que ajusta qué tanta importancia le da el agente a las recompensas futuras comparadas con las actuales, definiendo su visión estratégica.}. Imagínalo como un control de ``visión estratégica'': si $\gamma$ es cercano a 1, la IA tiene una visión de largo alcance y valora mucho los beneficios que llegarán en el futuro. Si es cercano a 0, la IA sufre de una ``miopía estratégica'' y solo le interesa ganar dinero en la próxima hora, ignorando lo que pase después.
...
\begin{itemize}
    \item \textbf{El Pilar Técnico (Binance):} Operamos \textbf{BNB} frente a \textbf{USDT}\footnote{Moneda estable (1:1 con dólar) utilizada como unidad de medida de rentabilidad.}. Elegimos la temporalidad de \textbf{1 hora} mediante la \textbf{vela horaria}\footnote{Representación gráfica del precio (apertura, cierre, máximos y mínimos) en intervalos de 60 minutos.} por ser el punto óptimo entre reactividad y reducción de ruido.
    
    \item \textbf{El Pilar de Datos:} La \textbf{API de Binance}\footnote{Protocolo de comunicación para recibir datos y ejecutar órdenes en tiempo real.} nos provee datos con alta \textbf{liquidez}\footnote{Facilidad para operar un activo sin afectar drásticamente su precio.}, garantizando que las decisiones de la IA sean ejecutables, considerando siempre la \textbf{volatilidad}\footnote{Medida de variación del precio; indica riesgo y oportunidad.} del mercado.

    \item \textbf{El Pilar Psicológico (\textbf{Alternative.me}\footnote{Plataforma de análisis de datos especializada en métricas del ecosistema criptográfico.}):} Integramos el \textbf{Crypto Fear \& Greed}\footnote{Indicador psicométrico (0-100) que cuantifica el sentimiento emocional del mercado.} Index. Esta fuente sintetiza volatilidad, redes sociales y tendencias de búsqueda en un solo indicador emocional.
\end{itemize}
...
Una pregunta clave es: ¿por qué usar Procesamiento de Lenguaje Natural (NLP) para el sentimiento en lugar de un indicador matemático tradicional? La respuesta reside en la naturaleza del mercado. Los indicadores técnicos (como medias móviles) son \textbf{deterministas}: se basan en fórmulas fijas que solo miran lo que ya pasó con el precio.
